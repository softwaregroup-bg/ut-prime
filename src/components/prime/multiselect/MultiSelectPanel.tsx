...
export const MultiSelectPanel = React.memo(React.forwardRef((props: any, ref) => {
    ...
    const createGroupChildren = (optionGroup) => {
        ...
        return (
            groupChildren.map((option, j) => {
                ...
                return (
                    <MultiSelectItem key={optionKey} label={optionLabel} option={option} template={props.itemTemplate} className={props.itemClassName}
    ...
    const createElement = () => {
        ...
        }, (props.columns > 0 && props.columns <= 12) && ('p-multiselect-col-' + props.columns), props.panelClassName);
